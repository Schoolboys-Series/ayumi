# TODO: Translation updated at 2018-01-27 16:27

# game/content/file_00003C75.rpy:85
translate japanese block_00003C84_93a1ccea:

    # rs_character_B69F854062C6466DB78FEEAEF86A4073 "哈哈，发生过这种事情呐。{w}\n嗯？{w=0.4}{nw}"
    rs_character_B69F854062C6466DB78FEEAEF86A4073 "うふふ。そう、そんなことがあったの。{w}\nん？{w=0.4}{nw}"

# game/content/file_00003C75.rpy:90
translate japanese block_00003C84_c9b24a25:

    # extend "不好，已经这个时间了。"
    extend "あらま、もうこんな時間じゃない。"

# game/content/file_00003C75.rpy:105
translate japanese block_00003C84_2b66c243:

    # rs_character_B69F854062C6466DB78FEEAEF86A4073 "友～！早上了～！早上了～！"
    rs_character_B69F854062C6466DB78FEEAEF86A4073 "友～！朝よ～！起きなさ～い！"

# game/content/file_00003C75.rpy:107
translate japanese block_00003C84_11680f22:

    # rs_character_B69F854062C6466DB78FEEAEF86A4073 "再见，忍君。{w}因为要去机场我必须走了。接下来就拜托了。"
    rs_character_B69F854062C6466DB78FEEAEF86A4073 "それじゃあ、しのぶくん。{w}飛行機の出発があるから、\n私はもう行くわね。あとはよろしくね。"

# game/content/file_00003C75.rpy:109
translate japanese block_00003C84_8a0b6daf:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "嗯，我明白。请走好，注意安全。"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "はい、分かりました。\nいってらっしゃい、お気をつけて。"

# game/content/file_00003C75.rpy:111
translate japanese block_00003C84_ab1a360e:

    # rs_character_B69F854062C6466DB78FEEAEF86A4073 "一直以来真是太感谢了。那我先走了。"
    rs_character_B69F854062C6466DB78FEEAEF86A4073 "いつも本当にありがとうね。\nじゃあ、行ってきます。"

# game/content/file_00003C75.rpy:151
translate japanese block_00003C84_853cf864:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "友，起床了。\n你妈妈已经走了。"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "こら友、起きろ。\nお母さん、もう出かけちゃったよ。"

# game/content/file_00003C75.rpy:157
translate japanese block_00003C84_8da11540:

    # rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "唔～哦～……{w}\n……{w=0.5}{nw}"
    rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "う～ん、ほ～い……。{w}\n……{w=0.5}{nw}"

# game/content/file_00003C75.rpy:165
translate japanese block_00003C84_d70071f3:

    # extend "再睡五分钟。"
    extend "あと５分。"

# game/content/file_00003C75.rpy:170
translate japanese block_00003C84_2c5bc439:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "不——行。已经不早了，没时间给你折腾。"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "だーめ。\n結構時間経っちゃって、あまり余裕が無いんだから。"

# game/content/file_00003C75.rpy:179
translate japanese block_00003C84_3892ce11:

    # rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "真是的～……外面太冷了不要不要～……{w=0.3}{nw}"
    rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "むう～ん……、お外寒いからイヤンイヤン～……。{w=0.3}{nw}"

# game/content/file_00003C75.rpy:183
translate japanese block_00003C84_e1f385a7:

    # extend ""
    extend ""

# game/content/file_00003C75.rpy:204
translate japanese block_00003C84_aed60ce0:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "别撒娇！你都多大了！一点都不可爱！\n"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "甘ったれるな！お前いくつだ！かわいくない！\n"

# game/content/file_00003C75.rpy:217
translate japanese block_00003C84_a65d7c4a:

    # extend "给你半分钟！起来！"
    extend "３０秒で支度しな！オラ早く！"

# game/content/file_00003C75.rpy:226
translate japanese block_00003C84_6e8389cb:

    # rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "噫——！忍好暴力～"
    rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "ひぇえええ、ドメスティックシノブ～。"

# game/content/file_00003C75.rpy:231
translate japanese block_00003C84_cace90e3:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "硬要形容这也只是“{color=#00FFFF}顾家的忍{/color}”而已。"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "それ、ただの{color=#00FFFF}『家庭的なしのぶ』{/color}だから。"

# game/content/file_00003C75.rpy:291
translate japanese block_00003C84_fe32bd42:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "准备好了？没落下东西吧？{w}好，那就去学校。"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "準備できた？忘れ物ない？{w}\nそれじゃあ、学校へ行くよ。"

# game/content/file_00003C75.rpy:496
translate japanese block_00003FB1_2515aeab:

    # rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}接下来会进入“学期末”（二周目）章节。{/color}"
    rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}これから二週目を進めます。{/color}"

# game/content/file_00003C75.rpy:498
translate japanese block_00003FB1_cafb2a5f:

    # rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}此章节内满足一定条件可重新进行一周目的故事。{/color}"
    rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}とある条件をクリアする時一週目のストーリーを\n一度体験することもできます。{/color}"

# game/content/file_00003C75.rpy:500
translate japanese block_00003FB1_fb8d9bd6:

    # rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}因此二周目也请记得随时保存。{/color}"
    rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}だからデータを保存することを忘れないでください。{/color}"

# game/content/file_00003C75.rpy:502
translate japanese block_00003FB1_1d99ac67:

    # rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}下面请保存你的{/color}{color=#FF0000}通关进度{/color}{color=#000000}。{/color}"
    rs_character_DB399286619A4AAB9D7D1A2D286772C8 "{color=#000000}ここまでの進行をセーブして保存してください。{/color}"

# game/content/file_00003C75.rpy:627
translate japanese block_00003C7B_e6ba867f:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "哈哈，不管什么时候看，这张照片的友都好有趣……"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "ふふっ。いつ見ても、この写真の友は面白いな……。"

# game/content/file_00003C75.rpy:629
translate japanese block_00003C7B_5cc328b5:

    # rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "欸——为什么——"
    rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "えー、どうしてだよー。"

# game/content/file_00003C75.rpy:631
translate japanese block_00003C7B_22b6a8d6:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "自己猜……{size=16}♪{/size}"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "さあね……{size=16}♪{/size}"

# game/content/file_00003C75.rpy:633
translate japanese block_00003C7B_2b7f97d0:

    # rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "那样的话我有更有趣的照片哦。"
    rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "あ、だったら\nオレもっと面白い写真持ってるよ。"

# game/content/file_00003C75.rpy:655
translate japanese block_00003C7B_d14d0666:

    # rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "呵呵！『忍{color=#FF8040}なめ太郎{/color}』 {size=12}{color=#C0C0C0}*日本一种吉祥物，样子请看忍的表情{/color}{/size}"
    rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "ンフンフ！『しのぶ{color=#FF8040}なめ太郎{/color}』！"

# game/content/file_00003C75.rpy:657
translate japanese block_00003C7B_707077a2:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "这不是友擅自设成我的LINE的头像的么。{w}就这么有意思？"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "コレ、友が勝手に、僕のRINEのアイコンにしたやつじゃん。{w}\nそんなに気に入ってるの？"

# game/content/file_00003C75.rpy:659
translate japanese block_00003C7B_cf79f1ac:

    # rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "哈哈！当然了。{w}哈哈！当然了真的和忍很像哦♪"
    rs_character_D45A9D2E09284CA0B5A11E1BF07A3CA2 "ンフンフ～。気に入ってるよ。{w}\nホラ、しのぶも真似して♪"

# game/content/file_00003C75.rpy:661
translate japanese block_00003C7B_d4088c77:

    # rs_character_D93E396B7ADB48CBAE13F206958FC08B "……呵呵～"
    rs_character_D93E396B7ADB48CBAE13F206958FC08B "……ンフンフ～。"# TODO: Translation updated at 2018-03-02 19:13

translate japanese strings:

    # content\file_00003C75.rpy:475
    old "{font=font/zcool-happy-ayumi-extended.ttf}第三章 完成{/font}"
    new "{font=font/honyaji-re.ttf}Chapter3 END{/font}"

    # game/content/file_00003C75.rpy:423
    old "{color=#42A5F5}ORIGINAL{/color}\nスクールボーイズ！\n見習い魔術師の任務！\nスクールボーイズ！歩\n{size=16}© 2016 Kiriya, Kasasagi{/size}\n\n{color=#42A5F5}CASTS{/color}\nTomo Moriumi\nShinobu Ayase\nTsubasa Ichinose\nSakuya Houmi\nTsuki Akamine\nSora Akamine\nShintarou Okumura\nSaburou Nekoyama\nShirou Nekoyama\nYukio Sakaki\nTsubasa-chan\nYuuhi\nNori\nMamoru Seigi\nKei Itou\nItsuki Kimura\nHikaru Satou\nNaoya Okajima\nTadashi Kojima\nJyunta Katou\nKenji Matsuda\nKakeru Izumi\nKobayashi\nMinami\nIsao Rikuta\nKokoro Sugimoto\nTakehito Kiyoshi\nKanon Nakayama\nYuusuke Okajima\nShion Nakayama\nMr. Nameko\nSyougintoki Suwabe\nSyouhei Suwabe\nYumi Nakajima\nOwner of Ramen shop\nOwner of Sushi shop\nOwner of Madchen Cafe\nOwner of Dagashi shop\nTentacle A\nTentacle B\nDark lesser panda\nMonster A\nMonster B\nMonster C\nOther monsters\nOctopus Alien\nKyubi\nSusumu Tokiwa\nKouya Sakase\nKai\nHe——\nBe——\n\n{color=#42A5F5}RESOURCES{/color}\nDOVA-SYNDROME\n音楽素材MusMus\nMusic is VFR\nポケットsound\n369\n音楽の卵\n小森平の使い方\n効果音ラボ\nGATAG (Steinway & Sons/Alan Alim)\n野田工房P\nOn-Jin ~音人~\n素材Good\nぴぽや\n\n{color=#42A5F5}OPENING THEME{/color}\n{size=30}Don't be afraid!!{/size}\n{size=16}by t.tam{/size}\n\n{color=#42A5F5}ENDING THEME{/color}\n{size=30}愛をかけて{/size}\n{size=16}by t.tam{/size}\n\n{color=#42A5F5}OPENING ANIMATION{/color}\nしょうちゃん\n\n{color=#42A5F5}PROGRAMMING{/color}\nLundarl Gholoi\nティルシー\n\n{color=#42A5F5}QUALITY ASSURANCES{/color}\nr98341034\n超级大白\n幽月艾\n凤箫剑\nTony Yang\n\n{color=#42A5F5}LOCALIZATION - CHINESE{/color}\nLundarl Gholoi\nr98341034\n电摩星人\n幽月艾\n"
    new "{color=#42A5F5}ORIGINAL{/color}\nスクールボーイズ！\n見習い魔術師の任務！\nスクールボーイズ！歩\n{size=16}© 2016 霧也, 鵲{/size}\n\n{color=#42A5F5}CASTS{/color}\n森海 友\n綾瀬 しのぶ\n一ノ瀬 つばさ\n穂海 作哉\n赤峰 月\n赤峰 空\n奥村 慎太郎\n猫山 三朗\n猫山 四朗\n榊 雪緒\nツバサ\nユウヒ\n朔\n世依木 マモル\n伊藤 圭\n木村 樹\n佐藤 光\n岡島 直弥\n小島 正\n加藤 準太\n松田 健治\n泉 翔\n小林\n南\n陸田 功\n杉本 志\n清 武人\n中山 花音\n岡島 雄介\n中山 紫音\n滑子先生\n諏訪部 翔銀時\n諏訪部 翔平\n由実おばさん\nラーメン屋店主\nお寿司屋店主\n本屋店主\nメッヒェンカフェ店主\n駄菓子屋店主\n触手 A\n触手 B\nダークレッサー\nお化け A\nお化け B\nお化け C\nお化け モブ\nタコタコ星人\n九尾\n常磐 進\n逆瀬 荒哉\n晦\nへー\nベー\n\n{color=#42A5F5}RESOURCES{/color}\nDOVA-SYNDROME\n音楽素材MusMus\nMusic is VFR\nポケットサウンド\n369\n音楽の卵\n小森平の使い方\n効果音ラボ\nGATAG (Steinway & Sons/Alan Alim)\n野田工房P\nOn-Jin ~音人~\n素材Good\nぴぽや\n\n{color=#42A5F5}OPENING THEME{/color}\n{size=30}Don't be afraid!!{/size}\n{size=16}by t.tam{/size}\n\n{color=#42A5F5}ENDING THEME{/color}\n{size=30}愛をかけて{/size}\n{size=16}by t.tam{/size}\n\n{color=#42A5F5}OPENING ANIMATION{/color}\nしょうちゃん\n\n{color=#42A5F5}PROGRAMMING{/color}\nLundarl Gholoi\nティルシー\n\n{color=#42A5F5}QUALITY ASSURANCES{/color}\nr98341034\n超级大白\n幽月艾\n凤箫剑\nTony Yang\n\n{color=#42A5F5}LOCALIZATION - CHINESE{/color}\nLundarl Gholoi\nr98341034\n电摩星人\n幽月艾\n"

