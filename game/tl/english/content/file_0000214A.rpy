# TODO: Translation updated at 2017-11-20 17:33

translate english strings:

    # content\file_0000214A.rpy:83
    old "背景曲"
    new ""

    # content\file_0000214A.rpy:83
    old "日常 1"
    new ""

    # content\file_0000214A.rpy:83
    old "这首曲子第一次出现是在序章中，但不仅仅在这里，\n而是出现在整个游戏的各个地方，恐怕是大家印象最深刻的曲子。\n旋律和第一章步模式的背景音乐有点相似，都是为了表现夏天的\n日常时光。"
    new ""

    # content\file_0000214A.rpy:83
    old "日常 2"
    new ""

    # content\file_0000214A.rpy:83
    old "与“日常 1”不同，本曲旋律相对低缓了一些，气势也不再那么\n高昂。如果説“日常 1”表现的是初中少年们朝气蓬勃的一面，\n“日常 2”便是他们度过的由一个个奇迹连接而成的平凡日常。"
    new ""

    # content\file_0000214A.rpy:83
    old "手足无措 1"
    new ""

    # content\file_0000214A.rpy:83
    old "真正的手足无措，以略带混乱四处跳跃的旋律演奏，完全不知道\n该从哪里听起的曲子。"
    new ""

    # content\file_0000214A.rpy:83
    old "手足无措 2"
    new ""

    # content\file_0000214A.rpy:83
    old "与其说“手足无措”，这首曲子更像是被逼的手足无措。从一开\n始不断重复的奇妙的上升旋律将整体代入了一个快节奏空间。不\n过背景音乐一直非常溷乱，结果还是不知道该从哪里听起。"
    new ""

    # content\file_0000214A.rpy:83
    old "平和 1"
    new ""

    # content\file_0000214A.rpy:83
    old "这是一首吉他为主钢琴为辅的曲子。平缓的曲调讲述的不是故事，\n而是每天我们司空见惯的一切。"
    new ""

    # content\file_0000214A.rpy:83
    old "平和 2"
    new ""

    # content\file_0000214A.rpy:83
    old "这是“平和 1”的后续，仍旧是那些乐器，仍旧是那个旋律，流\n逝而出的仍旧是如同空气一般理所应当的日常。"
    new ""

    # content\file_0000214A.rpy:83
    old "奇怪的酒店 1"
    new ""

    # content\file_0000214A.rpy:83
    old "中世纪舞曲，此处被用来表现大家入住的酒店的奇妙和脱离日常。\n略带宗教风格的乐曲更是加重了这一感受。"
    new ""

    # content\file_0000214A.rpy:83
    old "奇怪的酒店 2"
    new ""

    # content\file_0000214A.rpy:83
    old "相比刚入住时的激动和好奇，现在更多的则是冷静和对现状的把\n握。就算会发生什麽也不离奇的地方……"
    new ""

    # content\file_0000214A.rpy:83
    old "奇怪的酒店 3"
    new ""

    # content\file_0000214A.rpy:83
    old "周围越来越奇怪，时间的流逝似乎出现了问题，这并不是梦，这\n是现实。既然是现实，就必须给出解决方法。可是，一筹莫展的\n现在，自己真的还正常吗……"
    new ""

    # content\file_0000214A.rpy:83
    old "见鬼了 1"
    new ""

    # content\file_0000214A.rpy:83
    old "西洋宗教风，准确来说是基督教风格乐曲中用来描述恶魔场景的\n部分。鼓点和长号等低音乐器共同构筑了一幅尊贵的恶魔出场的\n画面。"
    new ""

    # content\file_0000214A.rpy:83
    old "见鬼了 2"
    new ""

    # content\file_0000214A.rpy:83
    old "主旋律使用了大量不稳定音，背景则充斥着腐朽的门窗等似乎是\n闹鬼老宅才会有的声音。就像真的要闹鬼一样。"
    new ""

    # content\file_0000214A.rpy:83
    old "朔的洞窟"
    new ""

    # content\file_0000214A.rpy:83
    old "和旋的音程非常不稳，不过主旋律却非常单纯。整体上来讲，和\n东方乐曲风格完全不同，确实很适合朔的身份。"
    new ""

    # content\file_0000214A.rpy:83
    old "悠长的时间"
    new ""

    # content\file_0000214A.rpy:83
    old "以提琴（小提琴和中提琴）为主的音乐。它所描述的时间，并不\n一定是现在，也可能只是回忆中的某一段。"
    new ""

    # content\file_0000214A.rpy:83
    old "无语的想法"
    new ""

    # content\file_0000214A.rpy:83
    old "比起“无语的想法”，这首曲子更适合被叫做“我无话可说”。\n总之就是对对方的言论不做评价，不予置评。"
    new ""

    # content\file_0000214A.rpy:83
    old "这是一个放松而又安心的地方，是只属于自己的领地，是最重要\n的避风港，是自己最重要的朋友每天都会来的地方。"
    new ""

    # content\file_0000214A.rpy:83
    old "主题曲"
    new ""

    # content\file_0000214A.rpy:83
    old "Don't be afraid!!"
    new ""

    # content\file_0000214A.rpy:83
    old "本作的片头曲。"
    new ""

    # content\file_0000214A.rpy:83
    old "Don't be afraid!! （伴奏）"
    new ""

    # content\file_0000214A.rpy:83
    old "本作片头曲的伴奏音轨。"
    new ""

    # content\file_0000214A.rpy:83
    old "愛を賭けて"
    new ""

    # content\file_0000214A.rpy:83
    old "本作的片尾曲。"
    new ""

    # content\file_0000214A.rpy:83
    old "第一章节"
    new ""

    # content\file_0000214A.rpy:83
    old "第一章步模式的背景音乐，有十足夏日气息的曲子。"
    new ""

    # content\file_0000214A.rpy:83
    old "第二章节"
    new ""

    # content\file_0000214A.rpy:83
    old "第二章步模式的背景音乐，由于天气转凉旋律也变得悠长了。"
    new ""

    # content\file_0000214A.rpy:83
    old "第三章节"
    new ""

    # content\file_0000214A.rpy:83
    old "第三章步模式的背景音乐。面对快要结束的学期，以及冬天的到\n来，心情也渐渐变得有些惆怅了。"
    new ""

    # content\file_0000214A.rpy:83
    old "学园活动"
    new ""

    # content\file_0000214A.rpy:83
    old "体育祭和音乐祭的背景音乐。不是很清楚为什么要放这种史诗风\n格的音乐。"
    new ""

    # content\file_0000214A.rpy:83
    old "赤峰兄弟的训练"
    new ""

    # content\file_0000214A.rpy:83
    old "和其他赤峰兄弟的曲子一样都是传统音乐风格。不过，因为这次\n涉及到训练的主题，曲调变得有些听起来很累（(⊙o⊙)…）"
    new ""

    # content\file_0000214A.rpy:83
    old "起点"
    new ""

    # content\file_0000214A.rpy:83
    old "本作主菜单的音乐。"
    new ""

    # content\file_0000214A.rpy:83
    old "充满朝气的夕阳下"
    new ""

    # content\file_0000214A.rpy:83
    old "描述了事件曲中“平和的午后”之后的时间。放学后，夕阳仍未\n西下，社团活动中，或是和朋友打闹中，不管怎样，一天都是充\n实的，都是无悔的，都是生命中珍贵的回忆。"
    new ""

    # content\file_0000214A.rpy:83
    old "英雄登场"
    new ""

    # content\file_0000214A.rpy:83
    old "标准的热血漫画和特摄片的节奏。如果真正的英雄降临人世，或\n许就是这样的感觉也説不定呢。"
    new ""

    # content\file_0000214A.rpy:83
    old "英雄末路"
    new ""

    # content\file_0000214A.rpy:83
    old "何为正义？何为邪恶？\n正义和邪恶真的是两个水火不容的词语吗？\n如果说这是神规定的，是否有一天我们会向扔给我们谜题的神举\n起武器呢？"
    new ""

    # content\file_0000214A.rpy:83
    old "御咲花车祭！"
    new ""

    # content\file_0000214A.rpy:83
    old "传统日本祭典的鼓点声。花车祭也是一个拥有一定历史的活动，\n以后也绝对会一直举办下去的！"
    new ""

    # content\file_0000214A.rpy:83
    old "漫才开始"
    new ""

    # content\file_0000214A.rpy:83
    old "没有什么“大阪风”之类的説法。硬要说的话就是流行感。作为\n具有十足幽默感和流行性的城市，这样上世纪末风格的音乐正是\n在描述大阪文化圈最为辉煌的时期，也是杉本和陆田的出身地。"
    new ""

    # content\file_0000214A.rpy:83
    old "天使与恶魔"
    new ""

    # content\file_0000214A.rpy:83
    old "基督教宗教风格音乐，不过并不传统，这是以现代乐器演奏的类\n似风格的音乐。单是听这首曲子的话，总觉得恶魔沾了上风。加\n油，恶魔！拿下友君！"
    new ""

    # content\file_0000214A.rpy:83
    old "Ailurus fulgens"
    new ""

    # content\file_0000214A.rpy:83
    old "小熊猫，拉丁语学名Ailurus fulgens。据说是一种毛茸茸、彭松松、\n无比可爱、令人无法自拔的动物。本游戏作者两人非常喜欢。"
    new ""

    # content\file_0000214A.rpy:83
    old "寂静之夜"
    new ""

    # content\file_0000214A.rpy:83
    old "全曲只有两种乐器，且都为敲击乐器，几乎没有任何和弦配合，\n所谓宁静的夜晚就是这样的。"
    new ""

    # content\file_0000214A.rpy:83
    old "宝物~卒業の春~"
    new ""

    # content\file_0000214A.rpy:83
    old "和朋友分别，踏上各自闪光的道路。然后，以自己的姿态绽放。\n希望能永远在你身边欢笑，希望能叫出你的名字。\n就算分开，我们也在同一片蓝天下。我绝对不会忘记你。\n真的，只有一瞬……我们毕业了。"
    new ""

    # content\file_0000214A.rpy:83
    old "事件曲"
    new ""

    # content\file_0000214A.rpy:83
    old "平和的午后"
    new ""

    # content\file_0000214A.rpy:83
    old "午后的时间是短暂的，却也难以忍耐，因为马上就要是放学时间。\n即便如此，放松心情慢慢等待也是值得的。"
    new ""

    # content\file_0000214A.rpy:83
    old "敌人来袭"
    new ""

    # content\file_0000214A.rpy:83
    old "乐曲中频繁出现早期电子游戏中常见的背景音乐的音符，彷佛就\n是遇敌的前兆。上昂的曲调则宣扬着战斗的意志。"
    new ""

    # content\file_0000214A.rpy:83
    old "最终决战"
    new ""

    # content\file_0000214A.rpy:83
    old "和上一首一样，仍是以怀旧感十足的早起电子乐为背景，不同的\n是鼓点动感十足。从中期开始旋律逐渐变得悠长，彷佛战斗的号\n角。末期的钢琴部分则诉说着战斗的间歇。"
    new ""

    # content\file_0000214A.rpy:83
    old "重重疑点"
    new ""

    # content\file_0000214A.rpy:83
    old "整曲充斥着不稳定音反复构成的旋律，主旋律也显得无比诡异。\n背景中几乎没有其他乐器参与，试图塑造一个安静空间。却因为\n无处不在的不稳定音变得越发诡异。事情的真相愈加扑朔迷离。"
    new ""

    # content\file_0000214A.rpy:83
    old "意料之外的展开"
    new ""

    # content\file_0000214A.rpy:83
    old "比起对意料之外的事件的惊讶，这首曲子主要表现的是发生后无\n所适从的情景。不过事情已经发生，除了觉得比较奇怪外也无话\n可说了。"
    new ""

    # content\file_0000214A.rpy:83
    old "微妙的气氛"
    new ""

    # content\file_0000214A.rpy:83
    old "犹如变魔术一般无法理解的气氛。即便从中期开始似乎有了那么\n一点头绪，最后还是没能搞明白现状。"
    new ""

    # content\file_0000214A.rpy:83
    old "暴风骤雨之前"
    new ""

    # content\file_0000214A.rpy:83
    old "通过弹拨乐器的简短挑拨发出的促音构成了乐曲的主旋律，比起\n一般来说的有力的感觉更偏向莫名其妙的感觉一些。叫做“暴风\n骤雨之前”是因为这在本作中等于搞事用的BGM。"
    new ""

    # content\file_0000214A.rpy:83
    old "感动的始末"
    new ""

    # content\file_0000214A.rpy:83
    old "这是温暖人心的曲调。平和缓慢的旋律构成了乐曲的基础，带来\n温暖的感觉。不过，这首曲子也用在了“寻求刺激·解”里……"
    new ""

    # content\file_0000214A.rpy:83
    old "追逐希望"
    new ""

    # content\file_0000214A.rpy:83
    old "轻快的步伐，灵动的音符，这是这首曲子带给我们的感觉。对未\n来和希望的憧憬与信心，正是它独一无二的旋律。"
    new ""

    # content\file_0000214A.rpy:83
    old "面向未来"
    new ""

    # content\file_0000214A.rpy:83
    old "每四拍一次的突然停顿让这首乐曲有了一步一步递进的感觉，背\n景乐器则诉説着平澹而又欢乐的日常。这就是通向未来的道路。"
    new ""

    # content\file_0000214A.rpy:83
    old "过去的宝物"
    new ""

    # content\file_0000214A.rpy:83
    old "宝物不一定都是美好的。即便是那些痛苦的回忆，也是我们一起\n度过的时光。或许正是因为那些才会有现在的我们。或许……\n或许这会是一生中难忘的回忆……\n谢谢……对不起……"
    new ""

    # content\file_0000214A.rpy:83
    old "过去的宝物……"
    new ""

    # content\file_0000214A.rpy:83
    old "这是友和忍的所有相关曲目中唯一一首电子乐，也是他们初次分\n道扬镳之时的背景音乐。与印象曲中的不同，旋律内充满了紧张\n感。这既是友的孤注一掷，也是忍第一次感受到背叛的滋味。"
    new ""

    # content\file_0000214A.rpy:83
    old "珍重的时光"
    new ""

    # content\file_0000214A.rpy:83
    old "本曲的主旋律可以分为前半段与后半段。前半段主要以钢琴为主，\n后半段则是电子乐器模拟的声音和钢琴混杂在一起。如果説一个\n是开心的时候，一个是难过的时候，那他们都是弥足珍贵的经历。"
    new ""

    # content\file_0000214A.rpy:83
    old "久远回忆"
    new ""

    # content\file_0000214A.rpy:83
    old "那是很久很久之前了。\n那时候的我们，还什么都不知道，还什么都不懂。\n我所记忆犹新的经历。\n自从那一天开始……"
    new ""

    # content\file_0000214A.rpy:83
    old "面向未来之前"
    new ""

    # content\file_0000214A.rpy:83
    old "从今以后，我们会向着各自不同的道路前行。\n道路终点的方向不尽相同，如果因此分离，也是无可奈何。\n慢慢习惯，渐渐适应，泯然日常……然后我们将会成为大人。\n正因如此，今后一起度过的而时光才会弥足珍贵，才更需要珍惜。"
    new ""

    # content\file_0000214A.rpy:83
    old "无比珍贵的现在"
    new ""

    # content\file_0000214A.rpy:83
    old "“只有经过努力才能看到彩虹”有人这麽説，但努力是有尽头的。\n筋疲力竭之时，愿意拉你一把的，才是真正的朋友，才是真正的\n宝物。"
    new ""

    # content\file_0000214A.rpy:83
    old "印象曲"
    new ""

    # content\file_0000214A.rpy:83
    old "赤峰兄弟 1"
    new ""

    # content\file_0000214A.rpy:83
    old "这首曲子更加侧重赤峰月，传统日本乐器、高昂的音调和悠扬的\n旋律共同塑造出了坚强开朗的少年这一形象。不过，背景里的其\n他乐器也是必不可少的，它们共同演奏着只属于他们的音符。"
    new ""

    # content\file_0000214A.rpy:83
    old "赤峰兄弟 2"
    new ""

    # content\file_0000214A.rpy:83
    old "除去鼓点外，这是一首由两种乐器演奏的音乐，一方负责主旋律，\n一方负责和弦，相辅相成，缺一不可。就旋律来看，这更适合作\n为日常曲，实际上，它也确实总出现在月和空单独相处的时候。"
    new ""

    # content\file_0000214A.rpy:83
    old "赤峰兄弟 3"
    new ""

    # content\file_0000214A.rpy:83
    old "非常古风的曲子，在赤峰兄弟的总共四首曲子中最有历史感。这\n是一首缅怀过去的乐曲。不过，缅怀过去是为了面向未来，这是\n毋庸置疑的。"
    new ""

    # content\file_0000214A.rpy:83
    old "九尾 1"
    new ""

    # content\file_0000214A.rpy:83
    old "平安时代的音乐风格，那也是妖怪和阴阳师的时代。采用日本传\n统乐器制作的这首以降调为主的曲子正是在叙述九尾辉煌的过去\n和微妙的现实。"
    new ""

    # content\file_0000214A.rpy:83
    old "九尾 2"
    new ""

    # content\file_0000214A.rpy:83
    old "近代日本的音乐风格，在传统中融入一定量西方元素，以悠扬的\n旋律体现历史的凝重。千年以来，九尾一直在此，直到那一天，\n它遇到……"
    new ""

    # content\file_0000214A.rpy:83
    old "九尾 3"
    new ""

    # content\file_0000214A.rpy:83
    old "风格重新回到古日本，不过和九尾的第一首印象曲不同，这首曲\n子非常明快。背景里偶尔出现的人声是神道教祭奠神灵的仪式，\n也就是祭典上常见的声音。九尾终于遇到了自己中意的人，可喜\n可贺，可喜可贺。"
    new ""

    # content\file_0000214A.rpy:83
    old "木村与伊藤"
    new ""

    # content\file_0000214A.rpy:83
    old "这是本作电子乐感最强的一首，其次就是三朗的印象曲。曲子本\n身不急不缓，电子乐构成的和弦却给乐曲本身添加了一层紧张感。\n值得一提的是，伊藤早已单恋木村很久，却因为对方有恋人而一\n直不敢追求。以对方的幸福优先也是伊藤温柔的地方。"
    new ""

    # content\file_0000214A.rpy:83
    old "猫山三朗"
    new ""

    # content\file_0000214A.rpy:83
    old "这也是野球拳场景的背景音乐。电子乐的大量出现使得这首曲子\n和本作其他乐曲存在较大差别，不过正好反映了三朗喜欢追赶潮\n流而又有些放荡不羁的性格。"
    new ""

    # content\file_0000214A.rpy:83
    old "奥村慎太郎"
    new ""

    # content\file_0000214A.rpy:83
    old "基于未知原因，三朗美发店用的就是慎太郎的印象曲，不得不説\n是真爱。曲子本身没有朔那么诡异，但也有点诡异，给人一种想\n要搞事的感觉。不过，作为“御咲学园HOMO计划”的主要负责\n人，这还是非常恰当的。"
    new ""

    # content\file_0000214A.rpy:83
    old "朔"
    new ""

    # content\file_0000214A.rpy:83
    old "朔的所有音乐都很诡异，这可能与他外星人的身份有关。背景中\n偶尔出现的狼的声音代表了他作为WOLFS一员的身份。整体偏西\n化和宗教化的曲风也彰显了他不为人知的神秘一面。"
    new ""

    # content\file_0000214A.rpy:83
    old "一之濑翼"
    new ""

    # content\file_0000214A.rpy:83
    old "这是一位胆小怕事而又活在过去的少年。\n他不善于交往，终于在中学遇到可能会成为朋友的人，但这个愿\n望也破灭了。他不再前进，不敢追求自己的幸福，过去的一切对\n他而言是最美好的时光。希望有一天他能得到救赎，面向未来。"
    new ""

    # content\file_0000214A.rpy:83
    old "森海友 1"
    new ""

    # content\file_0000214A.rpy:83
    old "这是友的第一首印象曲，从游戏开始便总是出现。友的印象曲\n中这首最为轻快，究竟这代表的是“轻快”还是“轻浮”就不\n好说了。"
    new ""

    # content\file_0000214A.rpy:83
    old "森海友 2"
    new ""

    # content\file_0000214A.rpy:83
    old "与友的其他印象曲一样，这是一首纯粹的钢琴曲。整曲音调偏\n高，变化不定，但都无比轻快。採用的和弦自始至终未曾变化，\n偶尔出现的不稳定音更是令乐曲本身更加贴近呆毛少年的本质。"
    new ""

    # content\file_0000214A.rpy:83
    old "森海友 3"
    new ""

    # content\file_0000214A.rpy:83
    old "作为友的三首印象曲中最正经的一首，它出现在第三章“变革\n进行曲“的部分，曲调也从之前的欢快为主变成了冷静的降调。\n这一次，他真的努力了，他真的做到了，这才是成为大人的必\n经之路。"
    new ""

    # content\file_0000214A.rpy:83
    old "友和忍"
    new ""

    # content\file_0000214A.rpy:83
    old "这是一首乍听非常混沌的曲子。不过从中途开始，突然出现的一\n股降调钢琴段落颠覆了这一印象，之后曲调没有变化，但主旋律\n已经被突出，成为整首曲子的主要部分。"
    new ""

    # content\file_0000214A.rpy:83
    old "作哉与翼"
    new ""

    # content\file_0000214A.rpy:83
    old "严格来说，这首曲子不是作哉和翼而是翼、作哉和小翼两人一狗\n的曲子。作哉是大家中生日最小的，却总被当做不良少年，但他\n其实非常温柔，就像这首曲子一样。希望他有朝一日能实现自己\n的愿望。"
    new ""

    # content\file_0000214A.rpy:83
    old "钢琴曲"
    new ""

    # content\file_0000214A.rpy:83
    old "Gymnopédie"
    new ""

    # content\file_0000214A.rpy:83
    old "来自埃里克·萨蒂早年的作品。这是一个希腊语和法语的合成词，\n描述了“活泼的孩子光着脚玩耍”的样子。可惜一到2eme\nGymnopédie同人社团这里，突然就把人家脱光变成全裸了……"
    new ""

    # content\file_0000214A.rpy:83
    old "La campanella"
    new ""

    # content\file_0000214A.rpy:83
    old "波兰舞曲《英雄》，被认为是肖邦作品中的杰作。"
    new ""

    # content\file_0000214A.rpy:83
    old "库普兰之墓"
    new ""

    # content\file_0000214A.rpy:83
    old "为纪念十八世纪法国作曲家库普兰而由拉威尔创作的《库普兰之\n墓》。"
    new ""

    # content\file_0000214A.rpy:83
    old "悲怆奏鸣曲"
    new ""

    # content\file_0000214A.rpy:83
    old "《La campanella》，意为“钟”，是由匈牙利作曲家弗朗茨·李\n斯特创作的《帕格尼尼大练习曲》中的一首钢琴独奏曲。"
    new ""

    # content\file_0000214A.rpy:83
    old "英雄"
    new ""

    # content\file_0000214A.rpy:83
    old "《Adagio Cantabile》，即贝多芬的《悲怆奏鸣曲》。"
    new ""

